...
class db_contrl:
  def __init__(self):
    self.credential_file = 'keys/db_creds'
    self.host = 'localhost'
    self.username = 'prettycool'
    self.password = ''
    self.db_name = 'db_data'
    self.charset = 'utf8mb4'

  def load_cred(self):
    print("[-] Loading database password")
    credential = open(self.credential_file)
  ...
  def key_control(self):
    query = "select * from tb_key;"
    if self.checkDB(query):
  ...
  def domainAdd(self,domain):
    ...
    if self.checkDB(query):
      query="INSERT INTO tb_domain(domainName,createdAt,cd_status) values ('{}', CURRENT_TIMESTAMP,001);".format(domain)
      self.commit(query)
      return True
    else:
      return False
