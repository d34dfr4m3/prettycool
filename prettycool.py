  ...
  for ips in range(len(data.json())):
    ...
    for ports in range(len(data.json()[ips]['iplList'])):
      try:
        ...
        print("\t[-] Port {}  Proto: {} Banner: {} ".format(port,protocol,banner))
...
def runMasscan(hostname,ipAddress):
  ...
  processHandler = subprocess.run(['masscan', ipAddress, '-Pn','--ports' ,'1-65535', '-oJ', fileName], stdout=FNULL,stderr=subprocess.STDOUT)
  FNULL.close()
  ...
  for host in range(len(payload)):
    print("[+] [MASSSCAN] Result for : {} ipAddress: {}".format(hostname,payload[host]['ip']))
